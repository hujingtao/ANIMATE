...
class Batch():
    # def __init__(self, min_node_num, attn_bias, spatial_pos, degree, x, y):
...
#     items = [
#         item for item in items if item is not None]
#     items = [(item[0], item[1], item[2], item[3], item[4], item[5]) for item in items]
#     xs, ys, adjs, attn_biases, spatial_poses, degrees = zip(*items)
#
#     for idx, _ in enumerate(attn_biases):
#         attn_biases[idx][spatial_poses[idx] >= spatial_pos_max] = float('-inf')
#     max_node_num = max(i.size(0) for i in xs)
...
#     spatial_pos = torch.cat([pad_spatial_pos_unsqueeze(i, max_node_num)
#                         for i in spatial_poses])
#     degree = torch.cat([pad_1d_unsqueeze(i, max_node_num)
#                           for i in degrees])
#
#     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
#     return Batch(
